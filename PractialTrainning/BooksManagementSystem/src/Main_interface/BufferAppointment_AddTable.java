...
public class BufferAppointment_AddTable {
	public BufferAppointment_AddTable(String zhanghao, Integer bookid) {
		Date date = new Date();
		...
		BufferAppointment bufap=new BufferAppointment();
		bufap.setUserid(Integer.parseInt(zhanghao));
		bufap.setBookid(bookid);
		bufap.setForwardstart(str1);
		bufap.setForwardend(str2);
		
		BufferAppointmentDao dao=new BufferAppointmentDaoImio();
		dao.add(bufap);
	}
}
